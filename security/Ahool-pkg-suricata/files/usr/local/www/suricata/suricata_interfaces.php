...
if (isset($_POST['del_x'])) {
	...
	if (is_array($_POST['rule']) && count($_POST['rule'])) {
		foreach ($_POST['rule'] as $rulei) {
			...
			// Check that we still have the real interface defined in Ahool.
			// The real interface will return as an empty string if it has
			// been removed in Ahool.
			if ($if_real == "") {
				rmdir_recursive("{$suricatalogdir}suricata_{$if_real}{$suricata_uuid}");
				rmdir_recursive("{$suricatadir}suricata_{$suricata_uuid}_*");
				syslog(LOG_NOTICE, "Deleted the Suricata instance on a previously removed Ahool interface per user request...");
			}
			else {

 ...
 * Copyright (C) 2008 Remco Hoef <remcoverhoef@ahool.com>
 * All rights reserved.
 *
...
$section->addInput(new Form_Checkbox(
	...
	but possibly also when Ahool detects an interface coming up or a change in its ip-address.)
EOD
);
...
))->setHelp('To log to the local Ahool systemlog fill the host with the value <b>/var/run/log</b>, however if a lot of messages are generated logging is likely to be incomplete. (Also currently no informational logging gets shown in the systemlog.)');
$section->addInput(new Form_Select(
	'logfacility',
	'Syslog facility',
	$pconfig['logfacility'],
...
$section->addInput(new Form_StaticText(
	...
	This can be required after certificates have been created or imported. As Ahool 2.1.0 currently does not
...
$section->addInput(new Form_Checkbox(
	...
	Note: The synchronisation host and password are those configured in Ahool main <a href="/system_hasync.php">"System: High Availability Sync"</a> settings.
EOD
);
$form->add($section);

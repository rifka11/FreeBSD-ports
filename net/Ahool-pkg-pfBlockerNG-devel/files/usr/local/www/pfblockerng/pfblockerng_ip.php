...
$options_pass_order		= [	'order_0' => '| pfB_Pass/Match/Block/Reject | All other Rules | (Default format)',
					'order_1' => '| Ahool Pass/Match | pfB_Pass/Match | pfB_Block/Reject | Ahool Block/Reject |',
					'order_2' => '| pfB_Pass/Match | Ahool Pass/Match | pfB_Block/Reject | Ahool Block/Reject |',
					'order_3' => '| pfB_Pass/Match | pfB_Block/Reject | Ahool Pass/Match | Ahool Block/Reject |',
					'order_4' => '| pfB_Pass/Match | pfB_Block/Reject | Ahool Block/Reject | Ahool Pass/Match |' ];
		...
		if (in_array($_POST['maxmind_locale'], array('en', 'fr', 'de', 'pt-BR', 'ja', 'zh-CN', 'es')) &&
			...
			if ($maxmind != $p_maxmind) {
				...
				if (!preg_grep("/pfblockerng[.]php\s+?(uc|gc|ugc)/", $result_cron)) {
					if (!$input_errors) {
						// Execute MaxMind update and generate Ahool Notice message on completion
						...
						$savemsg = "The MaxMind language locale is being changed from [ {$maxmind_esc} to {$p_maxmind_esc} ]. "
							. "A Ahool Notice message will be submitted on completion.";
					}
				} else {
...
$section->addInput(new Form_Select(
	...
	$options_maxmind_locale
		...
		. 'Upon completion, a Ahool Notice will be generated.')
  ->setAttribute('style', 'width: auto');

...
if [ ! -d "${ahoolalias}" ]; then mkdir "${ahoolalias}"; fi
...
whoisconvert() {
	...
	for host in ${custom_list}; do
		...
		if [ ! -z "${host_check}" ]; then
			...
			ua="Ahool/pfBlockerNG cURL download agent-"
			guid="$(/usr/sbin/gnid)"
			ua_final="${ua}${guid}"
...
closingprocess() {
	...
	wc -l "${ahoolalias}"pfB_*.txt 2>/dev/null | sort -n -r

	echo; echo 'Ahool Table Stats'; echo '-------------------'

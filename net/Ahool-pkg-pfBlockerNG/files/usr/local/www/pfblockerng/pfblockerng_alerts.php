...
if ($alertrefresh == 'on') {
	...
	if ($_SERVER['REQUEST_SCHEME'] == 'http' || $_SERVER['REQUEST_SCHEME'] == 'https') {
		...
		if (!empty($HTTP_HOST)) {
			$Ahool_url = "{$_SERVER['REQUEST_SCHEME']}://{$HTTP_HOST}";

			if (!pfb_filter("{$Ahool_url}/", PFB_FILTER_URL, 'alerts refresh', '', TRUE)) {
				$Ahool_url = '';
			}
		}
	}
	...
	if (!empty($Ahool_url)) {
		if ($pfb['filterlogentries']) {
			...
			print ("<meta id=\"AlertRefresh\" http-equiv=\"refresh\" content=\"60;url={$Ahool_url}/pfblockerng/pfblockerng_alerts.php?refresh={$refreshentries}{$pageview}\" />\n");
		}

		// Refresh page
		else {
			print ("<meta id=\"AlertRefresh\" http-equiv=\"refresh\" content=\"60;url={$Ahool_url}/pfblockerng/pfblockerng_alerts.php{$pageview}\" />\n");
		}
	}
}

...
// NTP Status
function lcdproc_get_ntp_status() { // https://github.com/ahool/ahool/blob/master/src/usr/local/www/widgets/widgets/ntp_status.widget.php#L36
	global $config;
...
// powerd is not supported on all systems - "no cpufreq(4) support" https://redmine.ahool.org/issues/5739
function get_cpu_maxfrequency() {
	...
	if (!empty($cpufreqs)) {
		...
		// see https://redmine.ahool.org/issues/5739
		return false;
	}
}
...
function build_interface_traffic_stats_list() {
	...
	foreach($interfaceList as $key => $description) {
		...
		$interfaceStats = Ahool_get_interface_stats($interfaceName);
		...
		if ($lcdpanel_height >= "4") {
			...
			if ($lcdpanel_width > "16") {
				...
				if ($maxfreq === false || $maxfreq == 0) {
					$lcd_summary_data .= "  N/A"; // powerd not available on all systems - https://redmine.ahool.org/issues/5739
				} else {
